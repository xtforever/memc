...
static void
m_puts(int query, char *s)
{
	m_write(query, m_len(query), s, strlen(s));
}

MYSQL *
m_mysql_connect(char *host, char *database, char *user, char *pwd)
{
	MYSQL *conn;
	...
	if (!(conn = mysql_init(NULL)))
		goto error;
	if (!(mysql_real_connect(conn, host, user, pwd, database, 0, NULL, 0)))
		goto error;
...
*/
int
m_mysql_query(MYSQL *conn, char *query, int res)
{
	char *s;
	MYSQL_RES *result;
	MYSQL_ROW row;
...
*/
int
m_mysql_create_insert_query(int buffer, char *tablename, int values, int row)
{
	int p, *d;
...
void
table_dump(MYSQL *mysql, char *tablename)
{
	int i, nrows;
	int p;
	char **strp;
...
int
get_field_by_index(int res, int fieldnum)
{
	char *s = v_get(res, "_RESFIELDS", fieldnum + 1);
	return v_lookup(res, s);
}
char *
get_entry(int res, int x, int y)
{
...
char *
field_name(int res, int i)
{
	return v_get(res, "_RESFIELDS", i + 1);
}

static char esctab[256] = { 0,
	                 ['\\'] = '\\',
	                 ['\0'] = '0',
	                 ['\n'] = 'n',
	                 ['\r'] = 'r',
...
static void
repl_char(int buf, char ch)
{
	if (esctab[(int)ch]) {
		m_putc(buf, '\\');
		m_putc(buf, esctab[(int)ch]);
		return;
	}
	m_putc(buf, ch);
}
...
int
mysql_quote_escape(int buf, int data)
{
	if (!buf)
		buf = m_create(m_len(data) * 2, 1);
	int p, *c;
	m_putc(buf, '\'');
	m_foreach(data, p, c) {
		if( !*c ) break;
